...
- Discrete vertical scrolling reveals three-column Moment grids, but their vertical position and final-row count vary by post text and ad placement. The runtime searches only the confirmed normalized content band `(0.057, 0.20, 0.780, 0.66)`, requires real image-content rows plus both white column gutters, and proposes only detected nonblank cells. The post action row and fixed Follow / Say Hi / Gift bar remain outside the search band.
- Live ScreenCaptureKit output must use the mirroring window's logical `418×920` size. Requesting an assumed Retina `836×1840` target produced a `418×920` image in the upper-left quadrant with black padding, invalidating normalized geometry.
- Feed dates such as `25/07` resemble viewer counters textually. A confirmed Moments tab plus Like/Comment feed anchors takes precedence over the counter regex. When viewer chrome times out and OCR sees only the status clock, a dark media frame plus the pagination strip provides the bounded visual fallback; a dark frame without pagination still fails closed.
- In-viewer Right Arrow, horizontal touch-drag, and discrete horizontal scroll all left the selected Moment unchanged. The reliable traversal is to dismiss the viewer and open each visible thumbnail cell directly.
- A downward viewer drag from approximately `(0.50, 0.48)` to `(0.50, 0.86)` closed both portrait and letterboxed landscape Moments after the transient `X` disappeared. Starting in the upper black letterbox was ignored, so the calibrated path begins in the universal image-center band and ends above the action row.
