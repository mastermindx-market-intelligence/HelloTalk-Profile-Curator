...
- A discrete vertical scroll on a Moments feed reliably revealed a full 3×3 thumbnail grid at normalized bounds approximately `(0.057, 0.357, 0.780, 0.349)`. The post action row begins below that grid and the fixed Follow / Say Hi / Gift bar remains farther down; both stay outside the thumbnail-safe region.
...
- A long downward viewer drag from approximately `(0.50, 0.27)` to `(0.50, 0.82)` closed the Moment viewer even after its transient `X` disappeared. Shorter drags were inconsistent, so only the long calibrated path should be proposed.
